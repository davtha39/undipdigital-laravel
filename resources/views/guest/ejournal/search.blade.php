...
<div class="single-news">
    ...
    <div class="container">    
        <div class="float-right"><br>
            <div class="b-search">
                <form action="{{route('guest.ejournal.search')}}" method="GET">
                    <input type="text" name="search" placeholder="Search" value="{{old('search')}}">
                    <button><i class="fa fa-search"></i></button>
                </form>
            </div>
        </div>
        <h1 class="sn-title">Hasil Penelusuran Data Ejournal</h1>
        Keyword: {{$search}}<br>
        <div class="row">
            <div class="col-lg-8">
                            ...
                            <div class="row news-card p-3">
                                ...
                                <div class="col-md-8">
                                    <div class="news-feed-text">
                                        <h5><a class="mailbox-attachment-name" style="color: black" href="{{route('guest.ejournal.show', $ejournals->ejournal_id)}}">{{$ejournals->judul}}</a></h5>
                                        <span>{{Str::limit($ejournals->deskripsi, 200)}} ...<br></span>
                                        <div class="float-right">
                                            <br>
                                            <h6> <i class="fas fa-solid fa-eye"></i>  {{$ejournals->view_count}}</h6>
                                        </div>
                                        <div class="d-flex flex-row justify-content-between align-items-center mt-2">
                                            <div class="d-flex creator-profile">
                                                @if ($ejournals->users->foto)
                                                    <img src="/foto_user/{{$ejournals->users->foto}}" alt="Admin" class="rounded-circle" width="45" height="45">
                                                @else   
                                                    <img src="/asset/defaultphoto.png" alt="Admin" class="rounded-circle" width="45" height="45">
                                                @endif
                                                <div class="d-flex flex-column ml-2">
                                                    <h6 class="username">{{$ejournals->users->name}}</h6>
                                                    <span class="date">{{$ejournals->created_at}}</span>
                                                </div><br>
                                            </div>
                                        </div>
                                    </div>
                ...
                Halaman {{ $ejournal->currentPage() }} dari {{ $ejournal->lastPage() }}<br>
            </div>
        </div>
    </div>
</div>
